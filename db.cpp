...
const uint32_t ROW_SIZE = 293;  // Change later to "const uint32_t ROW_SIZE = ID_SIZE + USERNAME_SIZE + EMAIL_SIZE;"
...
uint32_t* node_parent(void* node) { return reinterpret_cast<uint32_t*>(node) + PARENT_POINTER_OFFSET; }
...
void* get_page(Pager* pager, uint32_t page_num) {
    ...
    if (pager->pages[page_num] == NULL) {
        ...
        void* page = malloc(8192); // PAGE_SIZE WAS REPLACED BY 8192 TO MATCH THE NUMBER OF KB REQUIRED FOR 8 DIGITS
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
    ...
    */
    ...
    memcpy(left_child, root, 8192); // PAGE_SIZE WAS REPLACED BY 8192 TO MATCH THE NUMBER OF KB REQUIRED FOR 7 DIGITS
